...
function openLocalCrop(file, index) {
  ...
  const { el, close } = createOverlay({ className: 'cut', onClose: () => URL.revokeObjectURL(url) });
  recadrage(el, { src: url, alt: 'Photo à recadrer', sur: {
    annuler: close,
    valider: async sel => {
      try {
        await withBusy(async () => {
          const out = await decouper(file, sel);
      ...
      } catch (err) { console.warn('recadrage capture:', err); toast(`Recadrage échoué — ${humaniser(err)}. Réessaie.`, 'action'); }
    },
  } });
}
